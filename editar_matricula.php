...
if($id>0){
    ...
    $sql="select * from tb_matricula where id_matricula=$id";
    $result= mysqli_query($objConexion, $sql);
    if($result!=null){
        if(mysqli_num_rows($result)>0){
            $usuarioA= mysqli_fetch_array($result);
            $id_estudiante=$usuarioA['id_estudiantes'];
            $id_grasoso=$usuarioA['id_grado'];
             $id_jornad=$usuarioA['id_jornadas'];
             $id_pre=$usuarioA['id_periodo'];
            $paralelods=$usuarioA['id_paralelo'];
...
<html lang="en" dir="ltr">
  ...
  <title>Editar Matricula | UENDJ</title>
...
<div class="card card-default">
  <div class="px-6 py-4">
  <h2>Editar matricula <?php echo $id_estudiante ?></h2>
...
<div class="form-group col-6">
    ...
    <select name="id_estudiante" class="form-control estudinte" required>
...
$seleccionaperido='';
if($id_perido==$id_pre){
    $seleccionaperido='selected';
}
   ?>
     <option value="<?php echo $id_perido ?>" <?php echo $seleccionaperido ?>><?php echo $descrips ?></option>
<?php
   }
?> 
   </select>
...
$seleccionaparale='';
if($id_parael==$paralelods){
    $seleccionaparale='selected';
}
   ?>
   <option value="<?php echo $id_parael ?>" <?php echo $seleccionaparale?>><?php echo $descrip  ?></option>
...
<div class="form-group col-6">
   ...
   foreach($jornada as $jornadas){
        ...
        $seleccionaJord='';
        if($id_jorna==$id_jornad){
            $seleccionaJord='selected';
        }
   ?>
    <option value="<?php echo $id_jorna ?>" <?php  echo $seleccionaJord?>><?php echo $nombres ?></option>
<?php
   }
?>
   </select>
...
<div class="form-group col-6">
   ...
   foreach($curso as $cursos){
            ...
            $seleccionaGrado='';
            if($id_grabdi==$id_grasoso){
                $seleccionaGrado='selected';
            }
   ?>
    <option value="<?php echo $id_grabdi ?>" <?php echo $seleccionaGrado ?>><?php echo $nombre ?></option>
<?php 
}
?>
   </select>
...
$(document).ready(function() {
    ...
    $(".estudinte").select2({
        ...
        $('#nombre').val(selectedData.nombre);
        $('#codigo').val(selectedData.codigo);
        $('#apellido').val(selectedData.apellido);
    });

    var preSelectedId = <?php echo $id_estudiante; ?>;
    // Set the pre-selected student
    if (preSelectedId) {
        // Use AJAX to get the pre-selected student data
    
        $.ajax({
            url: "ajax/estudiante_json.php", // Adjust the URL accordingly
            dataType: 'json',
            data: {
                id: preSelectedId
            },
            success: function(data) {
                ...
                $(".estudinte").append(option).trigger('change');
               
                // Populate other fields based on the pre-selected student
